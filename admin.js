...
async function openEditDriver(driverId){ const {data}=await sb.from('profiles').select('*').eq('id',driverId).single(); if(!data) return; document.getElementById('driver-id').value=data.id; document.getElementById('driver-name-input').value=data.name||''; document.getElementById('driver-code-input').value=data.driver_id||''; document.getElementById('driver-phone-input').value=data.phone||''; document.getElementById('driver-active-input').value=String(data.is_active); document.getElementById('driver-email-group').style.display='none'; document.getElementById('driver-code-group').style.display=''; document.getElementById('driver-active-group').style.display=''; document.getElementById('driver-email-input').required=false; document.getElementById('modal-driver-title').textContent='Edit Driver'; openModal('modal-driver'); }
...
document.getElementById('btn-add-driver')?.addEventListener('click',()=>{ document.getElementById('form-driver').reset(); document.getElementById('driver-id').value=''; document.getElementById('driver-code-group').style.display='none'; document.getElementById('driver-active-group').style.display='none'; document.getElementById('driver-email-group').style.display=''; document.getElementById('driver-email-input').required=true; document.getElementById('modal-driver-title').textContent='Invite Driver'; openModal('modal-driver');});
document.getElementById('form-driver')?.addEventListener('submit', async (e)=>{ e.preventDefault(); const id=document.getElementById('driver-id').value; if(id){ const payload={name:document.getElementById('driver-name-input').value.trim(), phone:document.getElementById('driver-phone-input').value.trim()||null, is_active:document.getElementById('driver-active-input').value==='true', role:'driver'}; const {error}= await sb.from('profiles').update(payload).eq('id',id); if(error){toast(error.message,'error');return;} closeModal('modal-driver'); toast('Driver saved','success'); loadDriversTab(); return; } const fullName=document.getElementById('driver-name-input').value.trim(); const email=document.getElementById('driver-email-input').value.trim().toLowerCase(); if(!fullName||!email){ toast('Name and email are required','warning'); return; } const { error } = await sb.functions.invoke('driver-invite', { body: { email, fullName } }); if(error){toast(error.message,'error');return;} closeModal('modal-driver'); toast('Invite sent to driver email','success'); loadDriversTab();});
