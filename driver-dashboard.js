...
function parseStoredDocument(value) {
  ...
  if (typeof value === 'string') {
    ...
    try { return JSON.parse(value); } catch (_) { return { url: value }; }
  }
  return null;
}

async function driverDocumentLink(value, label = 'PDF') {
  const doc = parseStoredDocument(value);
  if (!doc) return '';
  if (doc.url && !doc.public_id) return `<a href="${doc.url}" target="_blank" rel="noopener">${label}</a>`;
  if (!doc.public_id) return '';
  const signedUrl = await getSignedUrl(doc.public_id, doc.resource_type || 'raw', true);
  return signedUrl ? `<a href="${signedUrl}" target="_blank" rel="noopener">${label}</a>` : `<span style="color:var(--text-muted)">${label} unavailable</span>`;
}

async function uploadPdfToCloudinary(pdfFile){
  if(!pdfFile) return null;
  const upload = await uploadToCloudinary(pdfFile, 'transroute/driver');
